    ...
    def verify(
        ...
        try:
            ...
            if not self.verification_id:
                logger.info("Requesting new verificationId ...")
            ...
            # 提交教师信息
            logger.info("Step 2/5: Submitting teacher info...")
            step2_body = {
            ...
            # 请求上传并上传文档
            logger.info("Step 4/5: Requesting upload URL ...")
            step4_body = {
                "files": [
                    {
            ...
            if step4_status != 200 or not isinstance(step4_data, dict) or not step4_data.get("documents"):
                raise Exception(f"Failed to get upload URL: {step4_data}")

            documents = step4_data["documents"]
            if len(documents) != len(assets):
                raise Exception("Upload tasks count does not match files count")
            ...
            logger.info(f"✅ Document submission complete: {getattr(step6_data, 'get', lambda k, d=None: d)('currentStep')}")
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        url = input("Enter SheerID verification URL (with externalUserId): ").strip()
    ...
    print(f"👉 Using URL: {verifier.install_page_url}")
